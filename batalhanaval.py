    ...
    if linha >= 0 and linha <= 4:
        ...
        if coluna >= 0 and coluna <= 9:
            ...
            if tabuleiroComputador[linha][coluna] == "🚢": #Se a string da posição escolhida for um ícone de navio
                ...
                print(f"Você tentou acertar uma posição onde já atirou, logo, perdeu a vez! Ainda restam {contadorNaviosRestantesInimigos} dele")
                sleep(2)
...
while contadorNaviosRestantesJogador != 0 and contadorNaviosRestantesInimigos != 0:

    contadorNaviosRestantesInimigos = adivinharInimigo(contadorNaviosRestantesInimigos)
    contadorNaviosRestantesJogador = adivinharComputador(contadorNaviosRestantesJogador)
    print("Cabo essa porra")
